...
function removeAndDouble(arr, startIdx) {
  ...
  arr.splice(startIdx); // Видаляємо елементи з масиву починаючи з заданого індексу
  var doubledArray = arr.map(function (element) {
    return element * 2; // Подвоюємо кожен елемент масиву
  });
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flattenedArray = arr.flat(); // Вирівнюємо масив

  const squaredArray = flattenedArray.map((num) => num ** 2); // Підносимо кожен елемент до квадрату

  const filteredSquaredArray = squaredArray.filter((num) => num >= threshold); // Фільтруємо елементи, які більше за порогове значення
